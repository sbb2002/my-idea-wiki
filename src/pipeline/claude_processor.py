...
- see_also: (선택) 사용자가 언급하지 않았더라도 이 아이디어와 관련 있어 함께 보면 좋은 개념, 기술, 프로젝트 등. 각 항목은 name(이름)과 description(한 줄 설명)으로 구성. 관련 항목이 없으면 빈 배열.
- content: 이번 주 업데이트 내용 — 노트의 핵심 아이디어, 인사이트, 발전 방향을 서술형으로 정리
- kickoff: 아이디어의 킥오프 문서. 노트 내용에서 추론 가능한 항목만 채우고, 알 수 없으면 빈 문자열로 둘 것.
  - core_value: 이걸 왜 만드는가? 한 문장으로. (추론 불가 시 "")
  - mvp_scope: 최소 구현 범위. 딱 이것만, 제외 항목도 명시. (추론 불가 시 "")
  - ui_anchor: 화면/인터페이스가 어떻게 생겼는지. (추론 불가 시 "")
  - tech_rationale: 기술 선택 근거. 익숙해서인가, 적합해서인가. (추론 불가 시 "")
  - weak_points: 구조적 약점 2개. (추론 불가 시 "")
  ...
  "items": [
    {
      ...
      "summary": "최신 요약...",
      "see_also": [
        { "name": "관련 개념 이름", "description": "한 줄 설명" }
      ],
      "content": "이번 주 업데이트 내용...",
      "kickoff": {
        "core_value": "핵심 가치 한 문장",
        "mvp_scope": "최소 구현 범위",
        "ui_anchor": "UI/인터페이스 설명",
        "tech_rationale": "기술 선택 근거",
        "weak_points": "구조적 약점 2개"
      },
...
def _build_user_prompt(notes: list[dict], existing_items: list[dict]) -> str:
    ...
    for i, note in enumerate(notes, 1):
        ...
        # 첨부 이미지 힌트 (pic_drive_id 있는 것만)
        drawings = note.get("_drawings", [])
        img_hint = ""
        if drawings:
            img_names = [d.get("filename") or f"drawing_{j+1}.jpg" for j, d in enumerate(drawings) if d.get("pic_drive_id")]
            if img_names:
                img_hint = "\n첨부 이미지: " + ", ".join(img_names)
        parts.append(
            f"=== 노트 {i}: {note['name']} ==={tag_hint}{img_hint}\n{note['content']}"
        )
    return "\n\n".join(parts)
...
BODY_SYSTEM_PROMPT = """당신은 아이디어 위키 문서의 상세 본문(body)을 작성하는 어시스턴트입니다.

주어진 아이디어 제목, 요약, 버전 히스토리를 바탕으로 상세 본문을 마크다운 형식으로 작성하세요.
- 소제목(##)과 본문으로 구성하되, 최소 2개 이상의 소제목 항목을 포함할 것
- 첨부 이미지가 있을 경우 body 내 적절한 위치에 ![설명](attachment:파일명) 형식으로 인라인 삽입 가능
- 요약을 단순 반복하지 말고, 아이디어의 배경/동기/구체적 구현 방향/고려사항 등을 풍부하게 서술할 것
- 순수 마크다운만 출력하세요. 다른 텍스트, 코드 펜스, 설명을 절대 포함하지 마세요."""


def generate_body_with_claude(
    title: str,
    summary: str,
    versions: list[dict],
    ...
    """
    아이템의 상세 본문(body)을 Claude로 생성한다. (2차 호출 — summary 변경 감지 시에만 호출)

    Args:
        title: 아이템 제목
        summary: 최신 요약
        versions: 버전 히스토리 목록 (최신순)
        attachments: 첨부 파일 목록 (OCR 텍스트 포함 가능)

    Returns:
        (body_markdown, tokens_used) 튜플
    """
    client = anthropic.Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))
    ...
    if versions:
        hist_lines = ["# 버전 히스토리 (최신순)"]
        for v in versions[:5]:  # 최근 5개만
            hist_lines.append(f"## {v.get('week', '')}\n{v.get('content', '')}")
        parts.append("\n".join(hist_lines) + "\n")

    if attachments:
        att_lines = ["# 첨부 이미지"]
        for att in attachments:
            if att.get("ocr_text") or att.get("description"):
                name = att.get("filename", "image")
                att_lines.append(f"- {name}: {att.get('description','')} {att.get('ocr_text','')}")
        if len(att_lines) > 1:
            parts.append("\n".join(att_lines) + "\n")

    parts.append("---\n위 정보를 바탕으로 상세 본문(body)을 작성하세요.")
    prompt = "\n".join(parts)
...
def _build_prd_prompt(item: dict, related_items: list[dict]) -> str:
    """
    PRD 생성용 프롬프트를 구성한다.

    Args:
        item: wiki.json 아이템 dict (summary, body, tags, attachments 포함)
        related_items: 연관 아이템의 title + summary 목록
    """
    parts: list[str] = []
    ...
    # 이미지 첨부 — OCR 텍스트 + 설명 포함 (시각 요소가 구현에 필요한 경우 대비)
    attachments = [a for a in item.get("attachments", []) if a.get("ocr_text") or a.get("description")]
    if attachments:
        att_lines = ["# Attached Visual References"]
        for att in attachments:
            name = att.get("filename", "image")
            ...
            if desc:
                att_lines.append(f"Description: {desc}")
            if ocr:
                att_lines.append(f"OCR Text:\n{ocr}")
        parts.append("\n".join(att_lines) + "\n")

    # 연관 아이템 — title + summary만 (토큰 절약)
    if related_items:
        rel_lines = ["# Related Items (for reference only — include only if architecturally essential)"]
        for rel in related_items:
            rel_lines.append(f"- **{rel['title']}**: {rel.get('summary', '')}")
        parts.append("\n".join(rel_lines) + "\n")

    parts.append(
        "---\n"
        "Generate a comprehensive PRD from the above information.\n"
        "Structure it with clear sections: Overview, Goals, Requirements, "
        "Technical Considerations, and any other sections relevant to implementation.\n"
...
def generate_prd(item: dict, related_items: list[dict] | None = None) -> str:
    """
    아이템의 PRD(Product Requirements Document)를 Claude로 생성한다.

    Args:
        item: wiki.json 아이템 dict
        related_items: related 필드로 연결된 아이템들의 title+summary 목록

    Returns:
        Markdown 형식의 PRD 문자열

    Raises:
        Exception: Claude API 실패 시
    """
    client = anthropic.Anthropic(api_key=os.environ.get("ANTHROPIC_API_KEY"))
    prompt = _build_prd_prompt(item, related_items or [])
