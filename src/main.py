...
_VIABILITY_PROMPT = """\
아래 아이디어 노트를 읽고, 네 가지 사실 확인 질문에 yes 또는 no로만 답하라.
추론하거나 빈 곳을 채워서 읽지 말 것. 명시적으로 적혀 있는 내용만 기준으로 판단하라.
이 질문들은 모두 필수 통과 조건이다. 확신이 없으면 no로 답하라.

질문:
1. 핵심 사용자 또는 타깃(플랫폼/환경/대상)이 구체적으로 명시되어 있는가?
   (예: "카카오톡 스크린샷", "웹 브라우저", "iOS 앱" 등 — "앱", "시스템", "모바일 메신저" 같은 미결정·추상적 표현은 no)
2. 기술 구현 접근법이 검증 없는 가정이 아니라 근거 있는 결정으로 기술되어 있는가?
   (사용할 기술/알고리즘/아키텍처가 선택 이유와 함께 적혀 있거나, 프로토타입 검증을 먼저 한다는 계획이 명시되어 있으면 yes —
    "색상 기반으로 하면 될 것이다" 처럼 검증 안 된 추측만 있거나, "자동화", "AI 활용" 수준의 추상적 표현이면 no)
3. 핵심 기능의 성공 판단 기준(인수 조건)이 적혀 있는가?
   (예: "크롭 결과가 말풍선 경계와 5px 이내로 일치", "10초 안에 결과 출력" — 무엇이 되면 완성인지 판단할 문장이 하나도 없으면 no)
4. MVP 완료를 판단할 수 있는 수준으로 범위가 구체적인가?
   (입력→출력 형태가 드러나고, 첫 버전에서 무엇을 만들고 무엇을 뺄지 가늠 가능하면 yes — 엣지 케이스나 예외 처리를 에이전트가 전부 임의로 채워야 하는 수준이면 no)
...
_VIABILITY_QUESTIONS = [
    "핵심 사용자/타깃(플랫폼/환경/대상)이 구체적으로 명시되지 않았습니다",
    "기술 구현 접근법이 검증 없는 가정 수준입니다 (근거 있는 결정 또는 프로토타입 검증 계획 필요)",
    "핵심 기능의 성공 판단 기준(인수 조건)이 없습니다",
    "MVP 완료를 판단할 수 있는 구체성이 부족합니다",
]
...
async def check_prd_viability(req: ViabilityRequest):
    ...
    1단계: 규칙 기반 (토큰 0)
    2단계: LLM 닫힌 질문 4개 (필수 통과 조건) — 1개라도 no면 부실
